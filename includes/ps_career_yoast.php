...
function register_custom_yoast_variables()
{
    ...
    wpseo_register_var_replacement('%%job_description%%', 'get_job_description', 'advanced',
        'Retrieve job describtion from Prescreen');
}
...
function filter_wpseo_canonical_param()
{
    ...
    if ($job) {
        return prescreeen_build_job_url($job);
    }
...
{
    return (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . parse_url($_SERVER["REQUEST_URI"],
            PHP_URL_PATH);
}
