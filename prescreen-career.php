...
require_once __DIR__ . "/includes/ps_career_rewrite_rules.php"; // Rewrite Rules
...
{
    add_menu_page('Prescreen Career', 'Prescreen Career', 'edit_pages', 'prescreen-career-settings',
        'prescreen_career_settings_function');
    add_submenu_page('prescreen-career-settings', 'Jobs', 'Jobs', 'edit_pages', 'jobs', 'list_jobs_table_page');
    add_submenu_page('prescreen-career-settings', 'Applications', 'Applications', 'edit_pages', 'applications',
        'list_application_table_page');
    add_submenu_page('prescreen-career-settings', 'All Forms', 'All Forms', 'edit_pages',
        '/edit.php?post_type=jobapplicationform', '');
    add_submenu_page('prescreen-career-settings', 'Add Form', 'Add Form', 'edit_pages',
        '/post-new.php?post_type=jobapplicationform', '');
    add_submenu_page('prescreen-career-settings', 'Custom Fields', 'Custom Fields', 'edit_pages',
        'application_custom_fields', 'list_application_cf_table_page');
    add_submenu_page('prescreen-career-settings', 'Formular Templates', 'Formular Templates', 'edit_pages',
        'application_custom_field_form_template', 'list_application_cf_template_table_page');
    add_submenu_page('prescreen-career-settings', 'Tags', 'Tags', 'edit_pages', 'application_tags',
        'list_application_tags_table_page');
    add_submenu_page("prescreen-career-settings", "Settings", "Settings", "manage_options", "prescreen_career_settings",
        "prescreen_career_options_page");
    add_submenu_page("prescreen-career-settings", "Logs", "Logs", "manage_options", "prescreen_career_logs",
        "list_logging_table_page");
...
if (defined('WP_CRON_CUSTOM_HTTP_BASIC_USERNAME') && defined('WP_CRON_CUSTOM_HTTP_BASIC_PASSWORD')) {
    function kb_http_basic_cron_request($cron_request)
    {
        ...
        $cron_request['args']['headers'] = isset($cron_request['args']['headers']) ? array_merge($cron_request['args']['headers'],
            $headers) : $headers;
        return $cron_request;
    }
...
function prescreen_career_enqueue_frontend_script()
{
    ...
    wp_enqueue_script('prescreen_career_js', plugin_dir_url(__FILE__) . 'assets/js/career-frontend-min.js?ver=1.8', '',
        '', true);
...
{
    if (!$fetchURL || !is_string($fetchURL) || !preg_match('/^http(s)?:\/\/[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(\/.*)?$/i',
            $fetchURL)) {
        return false;
    } else {
        $url = $fetchURL;
...
function importJobs()
{
    ...
    if (checkUrl($fetchURL) === true || checkUrl($fetchURL) != '') {
        ...
        $wpdb->query(
            $wpdb->prepare('UPDATE ' . $wpdb->prefix . 'prescreen_jobs_last_update SET lastupdate = %s WHERE id = 1',
                $date)
        );
        ...
        $wpdb->query($wpdb->prepare('UPDATE ' . $wpdb->prefix . 'prescreen_jobs_custom_data_fields SET status = %d',
            0));
        ...
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs_departments WHERE status = %d',
            0));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs_cities WHERE status = %d', 0));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs_positiontypes WHERE status = %d',
            0));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs_seniorities WHERE status = %d',
            0));
        ...
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs_custom_data_fields WHERE status = %d',
            0));
        ...
        if (googleIndexingIsActive()) {
            ...
            foreach ($googleIndexingDelete as $deleteShorthandle) {
                ...
                if (googleIndexing($deleteShorthandle, 'delete') === 200) {
                    ...
                    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs WHERE shorthandle = %s',
                        $deleteShorthandle));
                    ...
                    writeLogging('Google Indexing is active -> Response-Status: ' . googleIndexing($deleteShorthandle,
                            'delete') . '');
                    continue;
                }
            }
            ...
            foreach ($googleIndexingNew as $newShorthandle) {
                ...
                if (googleIndexing($newShorthandle, 'new') === 200) {
                    ...
                    writeLogging('Google Indexing is active -> Response-Status: ' . googleIndexing($newShorthandle,
                            'new') . '');
                    continue;
                }
            }
...
function updateGoogleIndexWithButton()
{
    ...
    foreach ($googleIndexingDelete as $deleteShorthandle) {
        ...
        if (googleIndexing($deleteShorthandle, 'delete') === 200) {
            ...
            $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_jobs WHERE shorthandle = %d',
                $deleteShorthandle));
            $returnGoogleIndexDel++;
        } elseif (googleIndexing($deleteShorthandle, 'delete') === 429) {
            $googleStatusCode = googleIndexing($deleteShorthandle, 'delete');
...
function job_snippet_function($atts)
{
    ...
    if (isset($_GET['sh']) || $_COOKIE['job_sh']) {
        if (isset($_COOKIE['job_sh'])) {
            $shortHandle = $_COOKIE['job_sh'];
        }
        if (isset($_GET['sh'])) {
            $shortHandle = $_GET['sh'];
        }
    ...
    foreach ($showArray as $showItem) {
        ...
        if (isset($matches[1])) {
            ...
            if ($showItem != 'custom_data_fields') {
                array_push($joinSelect,
                    "JOIN " . $wpdb->prefix . "prescreen_jobs_" . $showItemNew . " ON " . $wpdb->prefix . "prescreen_jobs" . "." . $showItem . " = " . $wpdb->prefix . "prescreen_jobs_" . $showItemNew . ".id ");
            }
        } else {
            ...
            if ($key === 'shorthandle') {
                ...
                if (strpos($key, 'custom_data_fields') !== false) {
                    ...
                    if (isset($databaseEntry['custom_data_field'][0])) {
                        foreach ($databaseEntry['custom_data_field'] as $cdf) {
                            ...
                            if (isset($cdf['id']) && $cdf['id'] === $cdfID) {
                                ...
                                if (!empty($cdf[$cdfField])) {
                                    $jobItemArray .= '<td class="' . preg_replace("/[^a-zA-Z]/", "",
                                            str_replace(' ', '',
                                                $cdf[$cdfField])) . ' custom_data_fields">' . $cdf[$cdfField] . '</td>';
                                } else {
                        ...
                        if (isset($cdf['id']) && $cdf['id'] === $cdfID) {
                            if (!empty($cdf[$cdfField])) {
                                $jobItemArray .= '<td class="' . preg_replace("/[^a-zA-Z]/", "", str_replace(' ', '',
                                        $cdf[$cdfField])) . ' custom_data_fields">' . $cdf[$cdfField] . '</td>';
                            } else {
                                $jobItemArray .= '<td class="custom_data_fields"></td>';
                            }
                    ...
                    if (isset($_GET["lat"]) && isset($_GET["lng"])) {
                        $jobItemArray .= '<td data-distance="' . vincentyGreatCircleDistance($_GET["lat"], $_GET["lng"],
                                $jobInfo[0],
                                $jobInfo[1]) . '" class="distance" data-lat="' . $jobInfo[0] . '" data-lng="' . $jobInfo[1] . '">' . vincentyGreatCircleDistance($_GET["lat"],
                                $_GET["lng"], $jobInfo[0], $jobInfo[1]) . '</td>';
                    } else {
                    ...
                    $jobItemArray .= '<td class="' . preg_replace("/[^a-zA-Z]/", "", str_replace(' ', '',
                            $jobInfo)) . ' ' . $selector . ' ' . $selectorAddon[0] . '">' . $jobInfo . '</td>';
                }
		...
		';
            ...
            if ($type === 'select') {
                ...
                if (strpos($by, 'custom_data_fields') !== false) {
                    ...
                    $template = $wpdb->get_results("SELECT " . $wpdb->prefix . "prescreen_jobs_" . $by . "." . $field . "" . $radiusSearchAddon . " FROM " . $wpdb->prefix . "prescreen_jobs JOIN " . $wpdb->prefix . "prescreen_jobs_" . $by . " ON " . $wpdb->prefix . "prescreen_jobs." . $selector . " = " . $wpdb->prefix . "prescreen_jobs_" . $by . ".id " . getWhereSQL($where,
                            $isValues, $isSelector) . " ORDER BY " . $field);
                }
                ...
                foreach ($template as $value) {
                    ...
                    if ($by === 'cities' && $field === 'title') {
                        ...
                        if (isset($value->title)) {
                            ...
                            array_push($returnArray,
                                '<option ' . $selectedClass . ' data-cityname="' . $value->title . '" value="' . $i . '" data-lat="' . $value->lat . '" data-lng="' . $value->lng . '" data-path=".' . preg_replace("/[^a-zA-Z]/",
                                    "", str_replace(' ', '', $value->title)) . '">' . $value->title . '</option>');
                        }

                    } else {
                        if (isset($value->$field)) {
                            array_push($returnArray,
                                '<option value="' . $i . '" data-path=".' . preg_replace("/[^a-zA-Z]/", "",
                                    str_replace(' ', '', $value->$field)) . '">' . $value->$field . '</option>');
                        }
                    }
...
function job_apply_button_function($atts)
{
    ...
    if ($type === 'api') {
        return '<div class="job__button"><a href="application/?sh=' . $short_handle . '&amp;sa=1' . $lang . '" ' . $target_op . ' ' . $rel_op . ' class="' . $class . ' button button--contained" style="' . $background_color_op . ' ' . $color_op . ' margin-top: 1em;">' . $title . '</a></div>';
    } else {
        return '<div class="job__button"><a href="application/?sh=' . $short_handle . '&amp;jh=' . $handle . '&amp;sa=1' . $lang . '" ' . $target_op . ' ' . $rel_op . ' class="' . $class . ' button button--contained" style="' . $background_color_op . ' ' . $color_op . ' margin-top: 1em;">' . $title . '</a></div>';
    }
...
function job_application_function()
{
    ...
    $startedApplication = '';
    if (isset($_GET['sa'])) {
        $startedApplication = $_GET['sa'];
    }
    $handle = '';
    if (isset($_COOKIE['job_jh'])) {
        $handle = $_COOKIE['job_jh'];
    }
    if (isset($_GET['jh'])) {
        $handle = $_GET['jh'];
    }
...
function google4Jobs_function()
{
    if (isset($_GET['sh'])) {
        ...
        if ($json_describiton) {
            $jsonld = $jsonld . ',' . PHP_EOL . '              "description" : "' . $json_describiton . '"';
        }
...
function importApplicationCustomFields()
{
    ...
    if ($apikey != '') {
        ...
        if (isset($array_appSource) && $array_appSource != '') {
            ...
            $wpdb->query($wpdb->prepare('UPDATE ' . $wpdb->prefix . 'prescreen_application_custom_fields SET is_active = %d',
                'false'));
            ...
            foreach ($array_appSource['data'] as &$appSource) {
                ...
                if (!empty($isInDatabase)) {
                    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_application_custom_fields WHERE cfid = %s',
                        $appSource->id));
                }
...
add_filter('acf/load_field/name=job_default_application_source',
    'acf_load_job_default_application_source_field_choices');
...
add_action('wp_ajax_manually_import_application_custom_field_form_templates',
    'importApplicationCustomFieldFormTemplates');
        ...
        if (isset($array_appSource) && $array_appSource != '') {
            ...
            if (isset($appSource['custom_field_values'])) {
                ...
                foreach ($appSource['custom_field_values'] as &$customFieldValue) {
                    ...
                    if (!empty($isInDatabase)) {
                        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_application_custom_field_form_template WHERE cfid = %s',
                            $cfid));
                    }
                    $wpdb->insert(
        ...
        if (isset($array_appSource) && $array_appSource != '') {
            ...
            if (isset($appSource['data'])) {
                ...
                foreach ($appSource['data'] as &$singleTag) {
                    ...
                    if (!empty($isInDatabase)) {
                        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_application_tags WHERE tagid = %s',
                            $tagid));
                    }
                    $wpdb->insert(
...
function deleteOldApplications()
{
    ...
    $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'prescreen_candidates WHERE created < %s',
        $currentDate));
    //wp_die();
}
        ...
        if (isset($shorthandle)) {
            ...
            for ($i = 0; $i < 50; ++$i) {
                if (isset($_FILES['job_applications_' . $i . ''])) {
                    ...
                    $job_applications_filetype = substr($job_applications_filetype,
                        strpos($job_applications_filetype, '/') + 1);
            ...
            if (isset($array_candidate['data'][0]->id) && $array_candidate['data'][0]->id != '') {
                ...
                saveCandidateToDatabase($jsonData, $candidatekey, 'exists', $application_url,
                    $array_candidate['data'][0]->id, $shorthandle, 'open');
                sendDoubleOptinMail($jsonData['profile']['firstname'], $jsonData['profile']['lastname'],
                    $jsonData['email'], $candidatekey, $userstate, $application_url);
                ...
                if (get_option('PS_job_applications_form_always_double_opt_in') != '') {
                    ...
                    // SEND DOUBLE OPTIN MAIL
                    $candidatekey = random_strings(9);
                    saveCandidateToDatabase($jsonData, $candidatekey, 'new', $application_url, '', $shorthandle,
                        'open');
                    sendDoubleOptinMail($jsonData['profile']['firstname'], $jsonData['profile']['lastname'],
                        $jsonData['email'], $candidatekey, $userstate, $application_url);
                    ...
                    saveCandidateToDatabase($jsonData, random_strings(9), 'new', $application_url, '', $shorthandle,
                        'unnecessary');
